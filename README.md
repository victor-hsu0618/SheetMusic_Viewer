...
ScoreFlow 重新定義了數位閱譜體驗，將傳統的 PDF 閱讀器轉化為具備「多重演奏詮釋比對」與「聲部協作」功能的專業音樂工作站。不論您是在交響樂團、室內樂團還是音樂教學場景，ScoreFlow 都能提供最具深度與流暢的解決方案。
...
### 🎭 2. 專業級註記與多重風格參考 (Professional Annotations)
*   **Notation Groups (專業圖層) & Sources**：將註記分為 Performance、Fingering、Bowing 等專業圖層，支援一鍵顯隱。
*   **Multi-Style Interpretation (多重演奏風格與詮釋)**：您可以同時疊加「大師詮釋」、「指揮提示」與「首席風格」。透過 **Ghosting (半透明比對)** 模式，在同一張譜上進行多維度的交叉閱讀。
...
*   **Peer Grabbing (一鍵抓取參考)**：看到團員分享的卡片？一鍵抓取並「疊加」為新的 Interpretation Style，即時對比首席的弓法設定。
