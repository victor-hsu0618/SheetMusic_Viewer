...
export class DriveFileManager {
    ...
    safeTitle(title) {
        ...
        // Strip legacy indexing prefix (e.g., "43-Concerto" -> "Concerto")
        // This resolves user confusion over "43-" prefixes generated by older versions or external syncs.
        let clean = title.replace(/^\d+-/, '').trim();
        
        const safe = clean.replace(/[/\\?*:|"<>]/g, '_').trim().slice(0, 100);
        return safe ? safe + '_' : '';
    }
