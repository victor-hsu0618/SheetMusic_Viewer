...
export class LayerManager {
    ...
    renderLayerUI() {
        ...
        this.app.layers.forEach(layer => {
            ...
            item.innerHTML = `
                <div class="layer-row-layout">
                    <div class="color-dot" style="background:${layer.color};" title="Tap to cycle color"></div>
                    <span class="layer-name">${layer.name}</span>
                    <span class="layer-count-value">${count > 0 ? count : ''}</span>
                    <div class="layer-btns">
                        <button class="layer-vis-btn ${layer.visible ? 'visible' : 'inactive'}" title="${layer.visible ? 'Hide' : 'Show'}">
                            ${eyeIcon}
                        </button>
                        ${count > 0 ? `
                        <button class="layer-erase-btn" title="Erase All in this Category">
